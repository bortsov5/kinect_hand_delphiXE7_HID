...
function GetBaudRate(Index: Integer): TBaudRate;
function GetParity(Index: Integer): TParity;
function GetStopbits(Index: Integer): TStopbits;
function GetDatabits(Index: Integer): TDatabits;
...
const
  SOpenError = 'Ошибка доступа к указанному com устройству';
  SInvalidHandle = 'Неверный дескриптор устройства, доступ запрещен';
  SPortAssigned = 'Устройство уже назначено (открыто)';
  SPortNotOpen = 'Порт не открыт, невозможно завершить операцию';
  SSetupCommErr = 'Ошибка инициализации буферов чтения/записи.';
  SUpdateDCBErr = 'Ошибка обновления DataControlBlock.';
  SCommTimeoutsErr = 'Ошибка обновления тайм-аутов связи.';
  SEscFuncError = 'Сбой функции Escape Comm';
  SMsgExt = ' (Ошибка: %d) ';
...
{TCommEventThread}
{$WARN SYMBOL_DEPRECATED ON}{!!! Resume}
...
procedure TCommEventChars.SetXonChar(Value: AnsiChar);
begin
...
procedure TCommEventChars.SetXoffChar(Value: AnsiChar);
begin
